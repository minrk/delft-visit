...
Any engines that have not run any tasks for the specified period will be shutdown.
"""
# Copyright (c) Min RK
...
def main():
    options.define('timeout', default=300,
        help="Time (in seconds) after which to consider an engine idle that should be shutdown.")
    options.define('interval', default=60,
        help="Interval (in seconds) at which state should be checked and culling performed.")
    options.parse_command_line()
    loop = ioloop.IOLoop.current()
    culler = EngineCuller(Client(), options.options.timeout)

    ioloop.PeriodicCallback(culler.update_state, options.options.interval * 1000).start()
    loop.start()
